 ...
 *      DEFINES
 *********************/
#define MY_CLASS &lv_cmenu_class
...
static void lv_cmenu_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj);
static void lv_cmenu_draw(lv_event_t *e);
static void lv_cmenu_event(const lv_obj_class_t *class_p, lv_event_t *e);
static void inv_cmenu_area(lv_obj_t *arc, lv_value_precise_t start_angle, lv_value_precise_t end_angle, lv_part_t part);
...
static bool lv_cmenu_angle_within_bg_bounds(lv_obj_t *obj, const lv_value_precise_t angle,
                                          const lv_value_precise_t tolerance_deg);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_cmenu_class = {
    .constructor_cb = lv_cmenu_constructor,
    .event_cb = lv_cmenu_event,
    .instance_size = sizeof(lv_cmenu_t),
...
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_cmenu_t *cmenu = (lv_cmenu_t *)obj;

    if (cmenu->active_item == active_item)
        return;

    int32_t new_item;
    new_item = active_item > arc->num_items ? arc->num_items : active_item;
    new_item = new_item < 0 ? 0 : new_item;

    if (cmenu->active_item == new_item)
        return;
    arc->active_item = new_item;
...
void lv_cmenu_set_num_items(lv_obj_t *obj, int32_t num_items)
{
    ...
    if (cmenu->num_items == num_items)
        return;
    cmenu->num_items = num_items;
    if (cmenu->active_item > num_items) {
        cmenu->active_item = num_items;
    }

    item_update(obj); /*value has changed relative to the new range*/
}
...
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    return ((lv_cmenu_t *)obj)->active_item;
}
int32_t lv_cmenu_get_num_items(const lv_obj_t *obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    return ((lv_cmenu_t *)obj)->num_items;
}
...
void lv_cmenu_align_obj_to_angle(const lv_obj_t *obj, lv_obj_t *obj_to_align, int32_t r_offset)
{
    ...
    lv_point_t center;
    int32_t cmenu_r;
    get_center(obj, &center, &cmenu_r);
    int32_t indic_width = lv_obj_get_style_arc_width(obj, LV_PART_INDICATOR);
    int32_t indic_width_half = indic_width / 2;
    cmenu_r -= indic_width_half;
    cmenu_r += r_offset;

    int32_t angle = (int32_t)get_angle(obj);
    int32_t knob_x = (cmenu_r * lv_trigo_sin(angle + 90)) >> LV_TRIGO_SHIFT;
    int32_t knob_y = (cmenu_r * lv_trigo_sin(angle)) >> LV_TRIGO_SHIFT;
...
void lv_cmenu_rotate_obj_to_angle(const lv_obj_t *obj, lv_obj_t *obj_to_rotate, int32_t r_offset)
{
    ...
    cmenu_r += r_offset;
    lv_obj_align_to(obj_to_rotate, obj, LV_ALIGN_CENTER, 0, -cmenu_r);
...
static void lv_cmenu_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    cmenu->rotation = 0;
    cmenu->bg_angle_start = 135;
    cmenu->bg_angle_end = 45;
    cmenu->indic_angle_start = 135;
    cmenu->indic_angle_end = 270;
    cmenu->min_close = 1;
    cmenu->min_value = 0;
    cmenu->max_value = 100;
    cmenu->dragging = false;
    cmenu->chg_rate = 720;
    cmenu->last_tick = lv_tick_get();
    cmenu->last_angle = cmenu->indic_angle_end;
    cmenu->in_out = CLICK_OUTSIDE_BG_ANGLES;
    cmenu->num_items = VALUE_UNSET;
    cmenu->active_item = VALUE_UNSET;
...
static void lv_cmenu_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    if (code == LV_EVENT_PRESSING)
    {
        ...
        if (cmenu->dragging == false)
        {
            ...
            {
                cmenu->dragging = true;
                cmenu->last_tick = lv_tick_get(); /*Capture timestamp at dragging start*/
            }
        }
        ...
        lv_value_precise_t bg_end = cmenu->bg_angle_end;
        if (cmenu->bg_angle_end < cmenu->bg_angle_start)
        {
            bg_end = cmenu->bg_angle_end + 360;
        }

        angle = lv_atan2(p.y, p.x);
        angle -= cmenu->rotation;
        angle -= cmenu->bg_angle_start; /*Make the angle relative to the start angle*/
        ...
        if (((min_close_prev == CLICK_CLOSER_TO_MIN_END) && (arc->min_close == CLICK_CLOSER_TO_MAX_END)) && ((CLICK_OUTSIDE_BG_ANGLES == arc->in_out) && (LV_ABS(delta_angle) > 280)))
        {
            angle = 0;
        }
        else if (((min_close_prev == CLICK_CLOSER_TO_MAX_END) && (arc->min_close == CLICK_CLOSER_TO_MIN_END)) && (CLICK_OUTSIDE_BG_ANGLES == arc->in_out))
        {
            angle = deg_range;
        }
        else
                                          ...
                                          const lv_value_precise_t tolerance_deg)
{
    ...
    else if ((360 < (bigger_angle + tolerance_deg)) && (angle <= 0 + ((bigger_angle + tolerance_deg) - 360)) && (angle > bigger_angle))
    {
